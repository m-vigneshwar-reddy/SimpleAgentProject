"""
Deprecated stub. The churn analyzer and sample data have been moved to the
`CustomerChurnPredictor` package at the repository root.

Run:
    python CustomerChurnPredictor/churn_analyzer.py

This stub keeps backward compatibility and avoids duplicate maintenance.
"""
import sys

print("This analyzer has moved to CustomerChurnPredictor/")
print("Run: python CustomerChurnPredictor/churn_analyzer.py")
sys.exit(0)
...
def analyze_account(a):
    """Analyze one account dict and return a structured CHURN RISK REPORT."""
    # helper funcs
    ...
    # Playbook suggestions (actionable)
    trigger = ''
    value_intervention = ''
    commercial = ''
...
def main():
    ...
    if JSON_FILE.exists():
        datasets.extend(parse_json(JSON_FILE))
    ...
    # print reports in required CHURN RISK REPORT format
    for r in reports:
        print('\n' + '='*60)
